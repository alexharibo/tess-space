...
/* ---------- frontpage scan reveal ---------- */
(function(){
  const STORAGE_KEY = "tess-scan-reveal-v1";   // <-- NYT
  const DEBUG_ALWAYS_PLAY = false;             // <-- NYT: sæt til true når du tester

  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const scanLine = document.getElementById('scanLine');
  const targets = Array.from(document.querySelectorAll('.scan-target'));

  // NYT: spring effekten over hvis den allerede er spillet
  const alreadyPlayed = !DEBUG_ALWAYS_PLAY && localStorage.getItem(STORAGE_KEY);

  if(reduceMotion || alreadyPlayed || !scanLine || !targets.length){
    if(scanLine) scanLine.remove();
    document.body.classList.remove('pre-scan');
    targets.forEach(el => el.classList.add('scanned')); // sørg for alt er synligt
    return;
  }

  document.body.classList.add('pre-scan');
  scanLine.style.opacity = '1';

  const DURATION = 1400;
  const endY = Math.min(window.innerHeight, 720);
  ...
  function tick(now){
    ...
    const y = easeInOutQuad(progress) * endY;
    ...
    if(progress < 1){
      ...
      targets.forEach(el => el.classList.add('scanned'));
      scanLine.style.transition = 'opacity 0.4s ease';
      scanLine.style.opacity = '0';

      // NYT: gem at effekten er spillet
      if(!DEBUG_ALWAYS_PLAY){
        try { localStorage.setItem(STORAGE_KEY, "played"); } catch(e) {}
      }
